<template>
  <v-app>
    <v-main>
      ...
      <v-container>
        <v-row>
          <v-col cols="12" md="6" v-if="foundation.length > 0">
            <v-card>
              <v-card-title>Foundation Courses ({{ foundation.length }})</v-card-title>              
              <v-divider></v-divider>
              <v-card-text>
                <v-list>
                  <v-list-item v-for="(item, index) in foundation" :to="{ name: 'CourseFeedback', params: { courseId: item.id } }">
                    <v-list-item-title>{{ item.name }}</v-list-item-title>
                    <v-list-item-subtitle>{{ item.id }} | Difficulty Score: {{ item.difficulty_rating }}</v-list-item-subtitle>
                  </v-list-item>
                </v-list>
              </v-card-text>
            </v-card>
          </v-col>
          <v-col cols="12" md="6" v-if="diploma.length > 0">
            <v-card>
              <v-card-title>Diploma Courses ({{ diploma.length }})</v-card-title>
              <v-divider></v-divider>
              <v-card-text>
                <v-list>
                  <v-list-item v-for="(item, index) in diploma" :to="{ name: 'CourseFeedback', params: { courseId: item.id } }">
                    <v-list-item-title>{{ item.name }}</v-list-item-title>
                    <v-list-item-subtitle>{{ item.id }} | Difficulty Score: {{ item.difficulty_rating }}</v-list-item-subtitle>
                  </v-list-item>
                </v-list>
          ...
          <v-col cols="12" md="6" v-if="degree.length > 0">
            <v-card>
              <v-card-title>Degree Courses ({{ degree.length }})</v-card-title>
              <v-divider></v-divider>
              <v-card-text>
                <v-list>
                  <v-list-item v-for="(item, index) in degree" :to="{ name: 'CourseFeedback', params: { courseId: item.id } }">
                    <v-list-item-title>{{ item.name }}</v-list-item-title>
                    <v-list-item-subtitle>{{ item.id }} | Difficulty Score: {{ item.difficulty_rating }}</v-list-item-subtitle>
                  </v-list-item>
                </v-list>
    ...
    <v-dialog v-model="editDialog" max-width="600">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-select v-model="editedLevel" :items="levelOptions" label="Level"></v-select>
          <v-select v-model="editedDpOrDs" :items="dpOrDsOptions" label="DP or DS"></v-select>
